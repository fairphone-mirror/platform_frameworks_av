...
public:
    ...
    {
        INT_PCM tmpOutBuffer[2048 * 2];

        // Flush the overlap-add buffer
        ALOGD("Flush AAC decoder");
        aacDecoder_DecodeFrame(this->mAACDecoder, tmpOutBuffer, sizeof(tmpOutBuffer), AACDEC_FLUSH);
        this->mLogging = true;
...
protected:
    ...
    uint8_t mDecOutput[2048 * MAX_CHANNEL_COUNT];
    bool mLogging;
};
